...
localAddAddress_xls = public.get_xls("userCase.xlsx", "addAddress")
...
def login():
    ...
    """
    # set url
    url = public.get_url_from_xml('login')
    ...
    token = public.get_value_from_return_json(response, "member", "token")
    return token
...
def logout(token):
    ...
    """
    # set url
    url = public.get_url_from_xml('logout')
    localConfigHttp.set_url(url)
